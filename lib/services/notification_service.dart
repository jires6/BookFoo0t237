...
''';
  ...
  Future<bool> _sendSmsViaTwilio(String phoneNumber, String otpCode) async {
    try {
      final message =
          'Votre code BookFoot237: $otpCode. Valide 5 min. Ne le partagez pas.';

      final response = await http.post(
        Uri.parse(
            'https://api.twilio.com/2010-04-01/Accounts/${NotificationConfig.twilioAccountSid}/Messages.json'),
        headers: {
  ...
  Future<String?> sendSmsOtpViaFirebase(
      String phoneNumber, Function onCodeSent) async {
    try {
  ...
  Future<bool> sendNewReservationNotificationToManager({
    ...
    try {
      print(
          '📧 Envoi notification nouvelle demande au gestionnaire: $managerEmail');
      print('📧 Depuis: ${NotificationConfig.gmailUsername}');
      print(
          '📧 Configuration Gmail active: ${NotificationConfig.isGmailConfigured}');
      ...
      final subject =
          '🔔 Nouvelle demande de réservation - $stadeNom - BookFoot237';
      final htmlContent = '''
      <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
      ...
      final sendReport = await send(message, smtpServer);
      print(
          '✅ Email de notification gestionnaire envoyé avec succès: ${sendReport.toString()}');
  ...
  Future<bool> sendCancellationNotificationToManager({
    ...
    try {
      ...
      final sendReport = await send(message, smtpServer);
      print(
          '✅ Email d\'annulation gestionnaire envoyé avec succès: ${sendReport.toString()}');
      return true;
    } catch (e) {
