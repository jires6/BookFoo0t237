...
class ImageUploadService {
  ...
  static Future<String?> uploadImage(XFile imageFile, String stadiumId) async {
    try {
      ...
      final Reference ref =
          _storage.ref().child('stadiums/$stadiumId/$fileName');
  ...
  static Future<List<String>> selectAndUploadImages(String stadiumId,
    ...
    try {
      print(
          '🖼️ ImageUploadService.selectAndUploadImages appelé pour stadiumId: $stadiumId');
      ...
      for (int i = 0; i < limitedFiles.length; i++) {
        ...
        print(
            '⬆️ Upload image ${i + 1}/${limitedFiles.length}: ${imageFile.name}');
      ...
      print(
          '🎉 Upload terminé. ${uploadedUrls.length}/${limitedFiles.length} images uploadées');
...
class _HomePageState extends State<HomePage> {
  ...
  Future<void> _deleteReservation(ReservationRequest request) async {
    try {
      ...
      print(
          '🗑️ ${isWaitingStatus ? "Suppression" : isRefusedStatus ? "Suppression définitive" : "Annulation"} de la réservation: ${request.stadeNom}');
...
class _EmailConfirmationPageState extends State<EmailConfirmationPage> {
  ...
  Future<void> _verifyOtpAndCreateAccount() async {
    ...
    try {
      // Vérifier l'OTP
      final bool isValid = NotificationService.instance
